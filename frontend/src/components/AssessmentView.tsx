...
export function AssessmentView({ projectId, onSnapshot, onSessionId, snapshotOverride }: { projectId: string; onSnapshot?: (snapshot: AssessmentSnapshot) => void; onSessionId?: (sessionId: string) => void; snapshotOverride?: AssessmentSnapshot }) {
  ...
  const [resume, setResume] = useState<ResumeContextSummary>(); const [resumeReady, setResumeReady] = useState(false); const [useResume, setUseResume] = useState(false); const [assessmentDepth, setAssessmentDepth] = useState<'QUICK' | 'STANDARD' | 'DEEP'>('STANDARD')
  ...
  const create = async (withResume = useResume) => { setBusy(true); setError(''); try { const result = await assessmentApi.create(projectId, undefined, withResume, { assessment_purpose: 'INTERVIEW_PRACTICE', assessment_depth: assessmentDepth }) as { id?: string; session_id?: string }; const id = result.id ?? result.session_id; if (!id) throw new Error('未返回测评会话'); setSessionId(id); onSessionId?.(id); await refresh(id) } catch (cause) { setError(cause instanceof Error ? cause.message : '创建测评失败') } finally { setBusy(false) } }
  ...
  if (!snapshot) {
    ...
    return <section className="assessment-setup" aria-labelledby="assessment-setup-title">
      ...
      <label><input type="checkbox" checked={useResume && Boolean(resume)} onChange={event => setUseResume(event.target.checked)} disabled={!resume} /> 使用当前简历作为背景参考</label><label>测评深度<select value={assessmentDepth} onChange={event => setAssessmentDepth(event.target.value as 'QUICK' | 'STANDARD' | 'DEEP')} disabled={busy}><option value="QUICK">快速</option><option value="STANDARD">标准</option><option value="DEEP">深度</option></select></label>
  ...
  return <div className="assessment-view"><AssessmentTimeline snapshot={snapshot} competencyNames={names} />{snapshot.retryable && <RetryNotice message="分析暂时失败，已保留你的回答。你可以先休息一下，准备好后再重试。" onRetry={retry} />}{snapshot.completion !== 'NONE' && <AssessmentCompletionCard completion={snapshot.completion} completedCount={snapshot.competencies.filter(c => c.status === 'SUFFICIENT' || c.status === 'EXHAUSTED').length} totalCount={snapshot.competencies.length} incompleteNames={snapshot.competencies.filter(c => c.status === 'INCOMPLETE').map(c => c.name)} />}{snapshot.status === 'IN_PROGRESS' && <>{busy && <ThinkingIndicator phase="Agent 正在分析你的回答" />}<AnswerComposer disabled={busy} question={snapshot.currentQuestion?.content} agentMessage={assistantNotices.at(-1) ?? ''} onSubmit={submit} /></>}</div>
}
