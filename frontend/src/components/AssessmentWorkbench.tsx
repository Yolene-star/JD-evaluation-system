...
export function AssessmentWorkbench({ snapshot, onPause, onResume, onFinish }: { snapshot: AssessmentSnapshot; onPause?: () => void; onResume?: () => void; onFinish?: () => void }) {
  ...
  return <div className="assessment-workbench">
    ...
    <div className="assessment-actions">{snapshot.status === 'IN_PROGRESS' && <><button onClick={onPause}>暂停</button><button onClick={onFinish}>结束测评</button></>}{snapshot.status === 'PAUSED' && <button onClick={onResume}>继续测评</button>}</div>
  </div>
}
